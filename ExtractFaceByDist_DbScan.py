...
def getPcds(folder,bag_file):
    # depthFrameList, colorFrameList, intrinsics, depth_scale = offline_acq(folder, 
    #                                                                         bag_file, 
    #                                                                         show_plt=False,
    #                                                                         save_frames=True,
    #                                                                       first_n=30)
    depthFrameList, colorFrameList, intrinsics, depth_scale = readByRSW(
        bag_file, usePlayBack=False, showPlt=False)
    ...
    for depthF, colorF in zip(depthFrameList, colorFrameList):
        pcdC = utils.rgbdToPcd(depthF,
                            ...
                            depth_scale)
        o3d.io.write_point_cloud(folder + '/show1' + ".ply", pcdC)
...
def DbScanClustering(pcd, numface, eps, min_points, folder):
    ...
    for label in range(min, max+1):
        ...
        if (len(label_pcd.points) == 15699 or len(label_pcd.points) == 10801):
            # 可视化
...
if __name__ == "__main__":
    folder = 'data/regtest/0726/face'
#     bag_file = folder + '/20230726_165352.bag'
#     getPcds(folder,bag_file)

    numface=0
    pcd = o3d.io.read_point_cloud(folder+'/scene.pcd')
    DbScanClustering(pcd,numface,eps=5,min_points=1000,folder=folder)
